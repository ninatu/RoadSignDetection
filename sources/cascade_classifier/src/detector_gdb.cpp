...
int main(int argc, char**argv )
{
    ...
    ifstream input_file;
    string name_image;
    input_file.open ("img.txt");

    while ( input_file >> name_image )
    {
        cout << name_image << endl;
        frame = imread(name_image);
        if( frame.empty() )
        {
...
void detectAndDisplay( Mat &frame, socket_t &socket, bool show)
{
    ...
    historySigns.process(frame, trafic_light.detect(frame, frame_gray), "traffic");
